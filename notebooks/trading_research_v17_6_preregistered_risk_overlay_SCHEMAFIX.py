...
def _validate_metric_ledger_schema(sim, strategy_name="UNKNOWN"):
  """Valida solo las columnas realmente exportadas por el ledger V17.5.2.1."""
  al = sim.get("daily_asset_ledger", pd.DataFrame())
  pl = sim.get("daily_portfolio_ledger", pd.DataFrame())

  if al.empty:
    raise ValueError(f"{strategy_name}: daily_asset_ledger vacio")
  if pl.empty:
    raise ValueError(f"{strategy_name}: daily_portfolio_ledger vacio")

  required_asset = {"date", "ticker", "shares_end", "allocated_cost"}
  missing_asset = sorted(required_asset - set(al.columns))
  if missing_asset:
    raise KeyError(
      f"{strategy_name}: daily_asset_ledger sin columnas obligatorias: "
      f"{missing_asset}. Disponibles: {sorted(al.columns)}"
    )

  has_direct_mv = "market_value_end" in al.columns
  has_reconstructable_mv = (
    "price_close" in al.columns or "last_valid_close" in al.columns
  )
  if not (has_direct_mv or has_reconstructable_mv):
    raise KeyError(
      f"{strategy_name}: no se puede valorar SHY al cierre. "
      "Se necesita market_value_end o price_close/last_valid_close."
    )

  required_portfolio = {"date", "equity_end"}
  missing_portfolio = sorted(required_portfolio - set(pl.columns))
  if missing_portfolio:
    raise KeyError(
      f"{strategy_name}: daily_portfolio_ledger sin columnas obligatorias: "
      f"{missing_portfolio}. Disponibles: {sorted(pl.columns)}"
    )
...
def _pct_invested(sim):
  """Porcentaje medio realmente invertido fuera de SHY, usando el ledger."""
  al = sim.get("daily_asset_ledger", pd.DataFrame()).copy()
  pl = sim.get("daily_portfolio_ledger", pd.DataFrame()).copy()

  _validate_metric_ledger_schema(
    sim,
    sim.get("metrics", {}).get("strategy", "UNKNOWN"),
  )

  al["date"] = pd.to_datetime(al["date"])
  pl["date"] = pd.to_datetime(pl["date"])

  shy = al.loc[al["ticker"].eq(CASH_ASSET)].copy()
  if shy.empty:
    return 100.0

  if "market_value_end" in shy.columns:
    shy["_market_value_end"] = pd.to_numeric(
      shy["market_value_end"], errors="coerce"
    )
  else:
    price = pd.Series(np.nan, index=shy.index, dtype=float)

    if "price_close" in shy.columns:
      price = pd.to_numeric(shy["price_close"], errors="coerce")

    if "last_valid_close" in shy.columns:
      price = price.fillna(
        pd.to_numeric(shy["last_valid_close"], errors="coerce")
      )

    shares_end = pd.to_numeric(
      shy["shares_end"], errors="coerce"
    ).fillna(0.0)
    shy["_market_value_end"] = shares_end * price.fillna(0.0)

  shy_mv = (
    shy.groupby("date", sort=True)["_market_value_end"]
    .sum(min_count=1)
    .rename("shy_market_value_end")
  )

  equity = (
    pl.assign(
      equity_end=pd.to_numeric(pl["equity_end"], errors="coerce")
    )
    .drop_duplicates("date", keep="last")
    .set_index("date")["equity_end"]
  )

  aligned = pd.concat([shy_mv, equity], axis=1).dropna()
  aligned = aligned[aligned["equity_end"] > 0]
  if aligned.empty:
    return np.nan

  invested_fraction = (
    1.0 - aligned["shy_market_value_end"] / aligned["equity_end"]
  ).clip(lower=0.0, upper=1.0)

  return round(float(invested_fraction.mean() * 100.0), 2)
...
# Validacion explicita del esquema antes de calcular las metricas agregadas.
for _schema_name, _schema_sim in overlay_sims.items():
  _validate_metric_ledger_schema(_schema_sim, _schema_name)
  _pct_value = _pct_invested(_schema_sim)
  if not np.isfinite(_pct_value):
    raise ValueError(f"{_schema_name}: pct_mean_invested no es finito")
print("POINT 7 LEDGER SCHEMA: PASS | pct_mean_invested calculable")
