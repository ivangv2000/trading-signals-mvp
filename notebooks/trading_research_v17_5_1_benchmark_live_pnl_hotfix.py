# %% [markdown]
# # Trading Research V17.5.1 — Benchmark, Live Signal and PnL Integrity Hotfix
#
# Corrige benchmark EW real, inferencia live XGB y contribución monetaria.
...
# %% [markdown]
# ## Subir archivos necesarios para V17.5.1
...
if missing:
  raise FileNotFoundError(
    "Faltan archivos obligatorios para V17.5.1: " + ", ".join(missing)
  )
...
# %% [markdown]
# ## 1. Configuracion congelada V17.5.1
...
rg = cfg["risk_gates"]
V175_REF = patch.get("v17_5_reference", {})
...
for name, sim in sims.items():
  ...
  asset_c, recon = calculate_realized_pnl_contribution(
    name, sim["targets"], data_dict, sim_equity=sim["equity"])
  if len(asset_c):
    contrib_all.append(asset_c)
    recon_all.append(recon)
    sector_contrib_all.append(sector_pnl_from_asset(asset_c).assign(strategy=name))
...
summary = {
  ...
  "universe_mode": "CURRENT_CONSTITUENTS_ASOF_LIQUIDITY",
  "v175_reference_status": V175_REF.get("FINAL_STATUS"),
  "v1751_champion_CAGR": champ_row["CAGR_pct"],
  "v1751_excess_CAGR_vs_B1": champ_excess,
  "v1751_information_ratio_vs_B1": champ_ir,
  "v1751_active_drawdown_pct": champ_row.get("active_drawdown_pct", np.nan),
  "v1751_max_stock_weight_pct": champ_row["max_stock_weight_pct"],
  "v1751_max_sector_weight_pct": champ_row["max_sector_weight_pct"],
  "v1751_max_drawdown_pct": champ_row["max_drawdown_pct"],
...
strategy_df.to_csv("research_v17_5_1_strategy_results.csv", index=False)
pd.DataFrame([summary]).to_csv("research_v17_5_1_summary.csv", index=False)
b1_audit_df.to_csv("research_v17_5_1_benchmark_audit.csv", index=False)
b1_holdings_df.to_csv("research_v17_5_1_benchmark_holdings.csv", index=False)
benchmark_results_df.to_csv("research_v17_5_1_benchmark_results.csv", index=False)
live_model_audit_df.to_csv("research_v17_5_1_live_model_audit.csv", index=False)
current_signals_df.to_csv("research_v17_5_1_current_signals.csv", index=False)
if len(contrib_df):
  contrib_df.to_csv("research_v17_5_1_contribution_by_asset.csv", index=False)
if len(sector_contrib_df):
  sector_contrib_df.to_csv("research_v17_5_1_contribution_by_sector.csv", index=False)
contribution_reconciliation_df.to_csv("research_v17_5_1_contribution_reconciliation.csv", index=False)
null_results_export.to_csv("research_v17_5_1_null_results.csv", index=False)
dsr_audit_df.to_csv("research_v17_5_1_dsr_audit.csv", index=False)
pd.DataFrame([{"gate": k, "pass": v} for k, v in validation_gates.items()]).to_csv(
  "research_v17_5_1_validation_gates.csv", index=False)
Path("research_v17_5_1_selected_config.json").write_text(json.dumps({
...
# %% [markdown]
# ## Empaquetar y descargar resultados V17.5.1

# %%
import zipfile

AUTO_DOWNLOAD_RESULTS = True
V1751_ZIP_PATH = CONTENT_ROOT / "V17_5_1_RESULTS.zip"


def build_v1751_results_zip(zip_path=None):
  zip_path = Path(zip_path or V1751_ZIP_PATH)
  root = CONTENT_ROOT
  with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for pattern in ("research_v17_5_1_*.csv", "research_v17_5_1_*.json"):
...
build_v1751_results_zip()
if AUTO_DOWNLOAD_RESULTS and IN_COLAB:
  from google.colab import files
  files.download(str(V1751_ZIP_PATH))
  print("Descarga iniciada: V17_5_1_RESULTS.zip")
elif AUTO_DOWNLOAD_RESULTS:
  print(f"AUTO_DOWNLOAD_RESULTS=True pero no estas en Colab. ZIP en: {V1751_ZIP_PATH}")

# %%
print("=" * 80)
print("REPORTE FINAL V17.5.1 BENCHMARK, LIVE SIGNAL AND PNL INTEGRITY HOTFIX")
print("=" * 80)
print(f"FINAL_STATUS: {FINAL_STATUS}")
print(f"Champion: {champion} | CAGR={champ_row['CAGR_pct']}% excess_B1={champ_excess}% IR={champ_ir}")
print(f"Benchmark: {PRIMARY_BENCHMARK} | Live: {LIVE_STRATEGY_SOURCE}")
...
print(f"Drawdown: {champ_row['max_drawdown_pct']}% | active_dd={champ_row.get('active_drawdown_pct')}%")
print(f"Null p vs B1={null_p:.6f} | PBO={prereg_pbo:.3f} | DSR prereg={prereg_dsr:.4f} global={global_dsr:.4f}")
print(f"PnL recon: {recon_ok} | Live scored={live_n_scored} | weight_sum={live_weight_sum:.6f}")
print(f"Universo: as-of rebalance | PIT={asof_pack['pit_membership_status']} | {SURVIVORSHIP_WARNING}")
print(f"V17.5 ref: {V175_REF.get('FINAL_STATUS')} -> V17.5.1: {FINAL_STATUS}")
...
# %%
# Recrea V17_5_1_RESULTS.zip desde exports existentes y descarga (sin re-ejecutar modelos).
import zipfile
from pathlib import Path
...
_ZIP_DL = _CONTENT_DL / "V17_5_1_RESULTS.zip"
_AUTO_DL = True


def _rebuild_v1751_zip(zip_path=None):
  zip_path = Path(zip_path or _ZIP_DL)
  root = _CONTENT_DL
  exports = list(root.glob("research_v17_5_1_*.csv")) + list(root.glob("research_v17_5_1_*.json"))
  if not exports:
    raise FileNotFoundError(
      "No hay exports research_v17_5_1_* en el directorio. Ejecuta primero el backtest completo."
    )
  with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
