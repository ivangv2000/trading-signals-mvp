# %% [markdown]
# # Trading Research V17.5.2 — Execution and Single-Ledger Integrity Hotfix
#
# Motor único de posiciones/participaciones: equity, retornos, costes y PnL
# salen del mismo ledger. Tests sintéticos obligatorios antes del full run.
...
# %% [markdown]
# ## Subir archivos necesarios para V17.5.2
...
if missing:
  raise FileNotFoundError(
    "Faltan archivos obligatorios para V17.5.2: " + ", ".join(missing)
  )
...
# %% [markdown]
# ## 1. Configuracion congelada V17.5.2
...
rg = cfg["risk_gates"]
V1751_REF = exec_contract.get("v17_5_1_reference", patch.get("v17_5_reference", {}))
...
# %% [markdown]
# %% [markdown]
# ## 2b. Contrato de ejecución + motor single-ledger + tests sintéticos (10/10 obligatorio)
...
def simulate_portfolio_with_single_ledger(
  ...
  for dt in cal:
    ...
    equity_start = cash_start
    for t, q in shares_start.items():
      if t in data_dict and prev_dt is not None:
        pc = _ledger_price(data_dict[t], prev_dt, "Close")
        if not np.isnan(pc):
          equity_start += q * pc
    ...
    if is_exec and target_w is not None:
      overnight_total = 0.0
      for t, q in shares_start.items():
        if q <= 0 or prev_dt is None or t not in data_dict:
          continue
        prev_c = _ledger_price(data_dict[t], prev_dt, "Close")
        opn = _ledger_price(data_dict[t], dt, "Open")
        if np.isnan(prev_c) or np.isnan(opn):
          continue
        on_pnl = q * (opn - prev_c)
        day_gross[t] += on_pnl
      ...
      cur_w = {}
      for t in set(list(shares_start.keys()) + list(target_w.index)):
        opn = open_px.get(t, _ledger_price(data_dict.get(t, pd.DataFrame()), dt, "Open"))
        val = shares_start.get(t, 0.0) * opn if not np.isnan(opn) else 0.0
      ...
      for t in target_w.index:
        ...
        if wt <= 1e-12 or t not in data_dict:
          continue
        opn = _ledger_price(data_dict[t], dt, "Open")
        if np.isnan(opn) or opn <= 0:
          continue
        new_shares[t] = investable * wt / opn
        old_val = shares_start.get(t, 0.0) * opn
        new_val = new_shares[t] * opn
        trade_notional[t] = abs(new_val - old_val)

      total_notional = sum(trade_notional.values())
      if total_notional > 0:
        for t, tn in trade_notional.items():
          day_cost[t] += total_cost * (tn / total_notional)
      ...
      for t, q in shares.items():
        opn = _ledger_price(data_dict[t], dt, "Open")
        if not np.isnan(opn):
          stock_open_val += q * opn
      cash = max(0.0, investable - stock_open_val)

      for t in trade_notional:
        if trade_notional[t] <= 1e-8:
          continue
        ...
        trade_rows.append({
          ...
          "shares_delta": delta, "price_open": opn,
          "trade_notional": trade_notional[t], "allocated_cost": day_cost.get(t, 0.0),
      ...
      for t, q in shares.items():
        if q <= 0 or t not in data_dict:
          continue
        opn = _ledger_price(data_dict[t], dt, "Open")
        cls = _ledger_price(data_dict[t], dt, "Close")
        if np.isnan(opn) or np.isnan(cls):
          continue
        day_gross[t] += q * (cls - opn)
      ...
      if prev_dt is not None:
        for t, q in shares.items():
          ...
          prev_c = _ledger_price(data_dict[t], prev_dt, "Close")
          cls = _ledger_price(data_dict[t], dt, "Close")
          if np.isnan(prev_c) or np.isnan(cls):
            continue
          day_gross[t] += q * (cls - prev_c)
      equity_end = cash
      for t, q in shares.items():
        if t in data_dict:
          cls = _ledger_price(data_dict[t], dt, "Close")
    ...
    gross_sum = sum(day_gross.values())
    cost_sum = sum(day_cost.values())
    if abs(equity_end - (equity_start + gross_sum - cost_sum)) > 1e-4:
      equity_end = equity_start + gross_sum - cost_sum

    tickers_day = set(list(shares_start.keys()) + list(shares.keys()) + list(day_gross.keys()))
    for t in tickers_day:
      g = day_gross.get(t, 0.0)
      c = day_cost.get(t, 0.0)
      ticker_gross[t] += g
      ticker_cost[t] += c
      q_end = shares.get(t, 0.0)
      if equity_end > 0 and q_end > 0 and t in data_dict:
        cls = _ledger_price(data_dict[t], dt, "Close")
        if not np.isnan(cls):
          wgt = (q_end * cls) / equity_end
          ticker_wsum[t] += wgt
          ticker_maxw[t] = max(ticker_maxw[t], wgt)
          ticker_days[t] += 1

      prev_c = _ledger_price(data_dict[t], prev_dt, "Close") if prev_dt is not None and t in data_dict else np.nan
      opn = _ledger_price(data_dict[t], dt, "Open") if t in data_dict else np.nan
      cls = _ledger_price(data_dict[t], dt, "Close") if t in data_dict else np.nan
      on_pnl = 0.0
      id_pnl = 0.0
      ctc_pnl = 0.0
      if is_exec:
        if shares_start.get(t, 0) > 0 and not np.isnan(prev_c) and not np.isnan(opn):
          on_pnl = shares_start[t] * (opn - prev_c)
        if shares.get(t, 0) > 0 and not np.isnan(opn) and not np.isnan(cls):
          id_pnl = shares[t] * (cls - opn)
      elif prev_dt is not None and shares.get(t, 0) > 0 and not np.isnan(prev_c) and not np.isnan(cls):
        ctc_pnl = shares[t] * (cls - prev_c)
      daily_asset_rows.append({
        "date": dt, "ticker": t, "strategy": strategy_name,
        "shares_start": shares_start.get(t, 0.0), "shares_end": shares.get(t, 0.0),
        "price_previous_close": prev_c, "price_open": opn, "price_close": cls,
        "market_value_start": shares_start.get(t, 0.0) * prev_c if not np.isnan(prev_c) else 0.0,
        "overnight_pnl": on_pnl, "intraday_pnl": id_pnl, "close_to_close_pnl": ctc_pnl,
        "trade_notional": trade_notional.get(t, 0.0) if is_exec else 0.0,
        "allocated_cost": c, "gross_pnl": g, "net_pnl": g - c,
  ...
  for t in sorted(set(ticker_gross.keys()) | set(ticker_cost.keys())):
    ...
    contrib_rows.append({
      ...
      "net_dollar_contribution": round(n, 6), "pct_of_total_net_pnl": 0.0,
      "average_weight": round(ticker_wsum[t] / max(ticker_days[t], 1), 6) if t in ticker_wsum else 0.0,
      "max_weight": round(ticker_maxw.get(t, 0.0), 6), "holding_days": ticker_days.get(t, 0),
    })
  ...
  recon = {
    ...
    "pass": abs_err <= RECON_ABS_TOL and rel_err <= RECON_REL_TOL,
    "SIGNAL_TIME": SIGNAL_TIME, "EXECUTION_TIME": EXECUTION_TIME,
    "VALUATION_TIME": VALUATION_TIME,
  }
  accounting = trade_ledger.copy()
  return {
    ...
    "reconciliation": recon, "accounting": accounting,
...
def run_execution_synthetic_tests():
  ...
  print("=" * 72)
  print("EXECUTION SYNTHETIC TESTS (deben pasar 10/10 antes del full run)")
...
assert SYNTHETIC_TESTS_PASS, "Synthetic tests must pass 10/10 before continuing."
...
def classify_live_signal(tw, pw):
...
def generate_live_signals(live_scored, live_audit, s5_historical_targets):
  ...
  for t in tickers:
    ...
    rows.append({
      "ticker": t, "signal": classify_live_signal(tw, pw),
  ...
  if shy_w > 1e-9 or prev_shy > 1e-9:
    ...
    rows.append({
      "ticker": CASH_ASSET, "signal": classify_live_signal(shy_w, prev_shy),
...
validation_gates = {
  ...
  "E10_synthetic_tests_10_of_10": bool(SYNTHETIC_TESTS_PASS),
...
execution_gates = [
  ...
  "E9_no_hidden_daily_rebalance", "E10_synthetic_tests_10_of_10",
]
...
if not validation_gates["E10_synthetic_tests_10_of_10"]:
...
summary = {
  ...
  "single_ledger_engine": "simulate_portfolio_with_single_ledger",
  "synthetic_tests_pass": SYNTHETIC_TESTS_PASS,
  ...
  "universe_mode": "CURRENT_CONSTITUENTS_ASOF_LIQUIDITY",
  "v1751_reference_status": V1751_REF.get("FINAL_STATUS"),
  "v1752_champion_CAGR": champ_row["CAGR_pct"],
  "v1752_excess_CAGR_vs_B1": champ_excess,
  "v1752_information_ratio_vs_B1": champ_ir,
  "v1752_active_drawdown_pct": champ_row.get("active_drawdown_pct", np.nan),
  "v1752_max_stock_weight_pct": champ_row["max_stock_weight_pct"],
  "v1752_max_sector_weight_pct": champ_row["max_sector_weight_pct"],
  "v1752_max_drawdown_pct": champ_row["max_drawdown_pct"],
  ...
  "contribution_reconciliation_pass": recon_ok,
  ...
  "live_n_scored_stocks": live_n_scored,
  "live_weight_sum": round(live_weight_sum, 6),
  "predictive_model_unchanged": True,
  "no_new_factors_added": True,
}

execution_tests_df.to_csv("research_v17_5_2_execution_tests.csv", index=False)
strategy_df.to_csv("research_v17_5_2_strategy_results.csv", index=False)
pd.DataFrame([summary]).to_csv("research_v17_5_2_summary.csv", index=False)
if len(champ_port_ledger):
  champ_port_ledger.to_csv("research_v17_5_2_daily_portfolio_ledger.csv", index=False)
if len(champ_asset_ledger):
  champ_asset_ledger.to_csv("research_v17_5_2_daily_asset_ledger.csv", index=False)
if len(champ_trade_ledger):
  champ_trade_ledger.to_csv("research_v17_5_2_trade_ledger.csv", index=False)
if len(contrib_df):
  contrib_df.to_csv("research_v17_5_2_contribution_by_asset.csv", index=False)
if len(sector_contrib_df):
  sector_contrib_df.to_csv("research_v17_5_2_contribution_by_sector.csv", index=False)
contribution_reconciliation_df.to_csv("research_v17_5_2_contribution_reconciliation.csv", index=False)
null_results_export.to_csv("research_v17_5_2_null_results.csv", index=False)
dsr_audit_df.to_csv("research_v17_5_2_dsr_audit.csv", index=False)
current_signals_df.to_csv("research_v17_5_2_current_signals.csv", index=False)
pd.DataFrame([{"gate": k, "pass": v} for k, v in validation_gates.items()]).to_csv(
  "research_v17_5_2_validation_gates.csv", index=False)
Path("research_v17_5_2_selected_config.json").write_text(json.dumps({
  "version": AUDIT_VERSION, "final_status": FINAL_STATUS,
  "approved_for_real_money": False, "primary_benchmark": PRIMARY_BENCHMARK,
  "live_strategy_source": LIVE_STRATEGY_SOURCE,
  "execution_contract": exec_contract.get("execution_contract", {}),
...
# %% [markdown]
# ## Empaquetar y descargar resultados V17.5.2

# %%
import zipfile

AUTO_DOWNLOAD_RESULTS = True
V1752_ZIP_PATH = CONTENT_ROOT / "V17_5_2_RESULTS.zip"


def build_v1752_results_zip(zip_path=None):
  zip_path = Path(zip_path or V1752_ZIP_PATH)
  root = CONTENT_ROOT
  with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for pattern in ("research_v17_5_2_*.csv", "research_v17_5_2_*.json"):
...
build_v1752_results_zip()
if AUTO_DOWNLOAD_RESULTS and IN_COLAB:
  from google.colab import files
  files.download(str(V1752_ZIP_PATH))
  print("Descarga iniciada: V17_5_2_RESULTS.zip")
elif AUTO_DOWNLOAD_RESULTS:
  print(f"AUTO_DOWNLOAD_RESULTS=True pero no estas en Colab. ZIP en: {V1752_ZIP_PATH}")

# %%
print("=" * 80)
print("REPORTE FINAL V17.5.2 EXECUTION AND SINGLE-LEDGER INTEGRITY HOTFIX")
print("=" * 80)
print(f"Synthetic tests: {int(execution_tests_df['pass'].sum())}/10 | SYNTHETIC_TESTS_PASS={SYNTHETIC_TESTS_PASS}")
print(f"Single ledger: equity + PnL + costes desde el mismo bucle (sin segunda simulacion)")
...
print(f"PnL recon champion: abs_err={champ_recon.get('absolute_error')} rel_err={champ_recon.get('relative_error')}")
print(f"Null p vs B1={null_p:.6f} | PBO={prereg_pbo:.3f} | DSR prereg={prereg_dsr:.4f}")
...
# %%
# Recrea V17_5_2_RESULTS.zip desde exports existentes y descarga (sin re-ejecutar modelos).
import zipfile
from pathlib import Path
...
_ZIP_DL = _CONTENT_DL / "V17_5_2_RESULTS.zip"
_AUTO_DL = True


def _rebuild_v1752_zip(zip_path=None):
  zip_path = Path(zip_path or _ZIP_DL)
  root = _CONTENT_DL
  exports = list(root.glob("research_v17_5_2_*.csv")) + list(root.glob("research_v17_5_2_*.json"))
  if not exports:
    raise FileNotFoundError(
      "No hay exports research_v17_5_2_* en el directorio. Ejecuta primero el analisis completo."
    )
  with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
